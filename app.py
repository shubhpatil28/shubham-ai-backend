...
# Force UTF-8 output encoding on Windows to prevent emoji crashes
...
# Initialize the new robust VoiceListener with SocketIO context
background_voice_listener = VoiceListener(socketio=socketio, ai_service=ai_service)
...
def handle_connect():
    ...
    # Emit initial state
    background_voice_listener.emit_state('listening' if background_voice_listener.is_listening else 'idle')
